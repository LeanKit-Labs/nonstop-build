...
function build( projectFn, buildFile, repoInfo, projectName, noPack, verbose ) {
	if ( _.isBoolean( projectName ) ) {
		noPack = projectName;
	}

	return when.promise( function( resolve, reject, notify ) {
		when.try( createProjects, projectFn, buildFile.get( repoInfo.path || repoInfo ), repoInfo, projectName, verbose )
					...
					when.settle( _.map( projects, function( project ) {
						return project.build( noPack )
							.progress( function( output ) {
								notify( { project: projectName, data: output } );
							} )
...
function createProjects( project, config, repoInfo, projectName, verbose ) {
	...
	if ( _.contains( platforms, sysInfo.platform ) ) {
		if ( projectName && config.projects[ projectName ] ) {
			...
			return _.map( config.projects, function( projectConfig, projectName ) {
				return project.create( projectName, projectConfig, repoInfo, verbose );
			} );
		}
	}
	return [];
